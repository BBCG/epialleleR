...
// Accepts data.table by reference, no type conversion required.
// Pairs and parses XM tags, outputs number of bases in HUXZhuxz contexts.
//

// [[Rcpp::export("rcpp_parse_xm")]]
std::vector<int> rcpp_parse_xm(Rcpp::DataFrame &df)                             // reference to data.table
{
  ...
  size_t x = 0, x0 = 0;                                                         // read counter, and it's value for the first read within the template
  ...
  for (x=0; x<xm.size(); x++) {
    ...
    // accumulate counts
    size_t l = xm[x].size();                                                    // XM length
    const char * s = (char *)xm[x];                                             // pointer to XM c-string
