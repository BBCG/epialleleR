...
Rcpp::DataFrame rcpp_extract_patterns(Rcpp::DataFrame &df,                      // data frame with BAM data
  ...
  for (unsigned int x=0; x<rname.size(); x++) {   
    ...
    if (rname[x]==target_rname) {
      ...
      const unsigned int end_x = start[x] + size_x - 1;
      signed int overlap = std::min(end_x, target_end) - std::max(start[x], target_start) + 1;
...
/*** R
rcpp_extract_patterns(bam, 47, 43124861, 43125261, 1, "zZ", TRUE)
*/
