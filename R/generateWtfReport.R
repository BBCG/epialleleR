...
#'
#' @description
#' This function computes WTF per genomic position.
#'
#' @details
#' The function reports WTF at the level of
#' individual cytosines using BAM file or data as an input. Function uses the
#' following formula:
#' 
#' \deqn{WTF=\frac{\sum_{i=1}^{l} w_{i} \times MH_{i}}{\sum_{i=1}^{l} w_{i} \times H_{i}}}
#' 
#' Methylated Haplotype Load (MHL) formula first described by Guo et al., 2017
#' (doi: \href{https://doi.org/10.1038/ng.3805}{10.1038/ng.3805}): 
#' 
#' \deqn{MHL=\frac{\sum_{i=1}^{l} w_{i} \times P(MH_{i})}{\sum_{i=1}^{l} w_{i}}}
#' 
#' where \eqn{l} is the length of a haplotype, \eqn{P(MH_{i})} is the fraction
...
#' how WTF formula is applied. These parameters are sought to make
#' WTF calculations more flexible for various use case scenarios. These
...
#'                report.context="CX")
#' @export
generateWtfReport <- function (bam,
                                    report.file=NULL,
