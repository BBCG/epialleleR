...
- threshold.reads:
  ...
  thresholding makes the report virtually indistinguishable from the
  ones generated by other software, such as Bismark or Illumina DRAGEN
  Bio IT Platform. Thresholding is **not** recommended for long-read
  sequencing data.
...
- max.outofcontext.beta:
  ...
  considered completely unmethylated (all C are counted as T). This
  option has no effect when read thresholding is disabled.

- report.context:

  string defining cytosine methylation context to report (default: value
  of \`threshold.context\`).
...
the thresholding parameters (threshold.reads = TRUE, threshold.context =
"CG", min.context.sites = 2, min.context.beta = 0.5,
...
leading to the following CX report (given that all reads map to
chr1:+:1-16):
...
With the thresholding disabled (threshold.reads = FALSE) all methylated
bases will retain their status, so the CX report will be similar to the
reports produced by other methylation callers (such as Bismark or
...
having more than 10 cytosines methylated, will be effectively treated as
unmethylated ones.
